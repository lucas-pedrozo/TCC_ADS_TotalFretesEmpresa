...
const AdminCompaniesPage = () => {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data } = await http.get<AdminCompany[]>("/company");
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (company) =>
        ...
        company.cnpj?.includes(q)
    );
  }, [items, search]);

  return (
    <AdminPageShell
      title={t("pages.admin.companies.title")}
      description={t("pages.admin.companies.description")}
    >
