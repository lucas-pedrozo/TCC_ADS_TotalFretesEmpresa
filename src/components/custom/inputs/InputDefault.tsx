...
export const INPUT_STYLES = {
  default: {
    ...
    input:
      "w-full p-2.5 text-black text-base bg-white border border-black/60 rounded-lg placeholder:text-black/60 focus:outline-none",
  },
  error: {
    ...
    <Controller
      ...
      render={({ field, fieldState: { error } }) => (
        <div className="flex flex-col gap-1">
          <label
            htmlFor={name}
            className={disabled ? INPUT_STYLES.disabled.label : error ? INPUT_STYLES.error.label : INPUT_STYLES.default.label}
          >
            {label}
          </label>
          ...
          <input
            ...
            maxLength={maxLength}
            disabled={disabled}
            className={disabled ? INPUT_STYLES.disabled.input : error ? INPUT_STYLES.error.input : INPUT_STYLES.default.input}
          />

          {error && <span className="pl-2.5 text-red-500 text-sm">{error.message}</span>}
        </div>
      )}
    />
  )
