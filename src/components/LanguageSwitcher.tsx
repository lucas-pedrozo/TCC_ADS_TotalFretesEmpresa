  ...
  return (
    <div className="fixed right-4 bottom-4 z-50">
      <div className="relative">
        {isOpen && (
          <div className="absolute right-0 bottom-12 w-44 rounded-md border border-border bg-popover shadow-lg">
            {SUPPORTED_LANGUAGES.map((option) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left text-sm transition-colors hover:bg-muted ${
                  option === language ? "font-semibold text-foreground" : "text-muted-foreground"
                }`}
              >
                {t(`switcher.${option}`)}
              </button>
